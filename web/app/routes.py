...
from azure.servicebus import Message, ServiceBusClient, QueueClient
...
def notification():
    if request.method == 'POST':
        ...
        try:
            ...
            #################################################
            notification = Notification.query.filter_by(submitted_date=notification.submitted_date).first()
            try:
